...
const jsonFile = "..\\..\\Output\\project_ast.json";
...
function update(source) {
	...
	const nodeEnter = nodeSel
		...
		.on("dblclick", (event, d) => {
			// double click node will redirect to the file in VSCode
			showCodeSnippet(d);
		})
		.on("mouseover", (event, d) => {
			// show information about the node
			const snippet = d.data.text
				? d.data.text.substring(0, 200) +
				  (d.data.text.length > 200 ? "..." : "")
			...
			tooltip
				...
				.html(
					`<b>Type:</b> ${d.data.type || "N/A"}<br>` +
						...
						`<b>Start line:</b> ${
							d.data.startLine || "?"
						}, <b>End line:</b> ${d.data.endLine || "?"}<br>` +
	...
	const linkSel = g.selectAll(".link").data(links, (d) => d.target.id);
